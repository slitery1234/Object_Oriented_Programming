...
// BFS to find route
int BFS(int n,vector<int> graph[],int start,int dest,int CDS[],node nodes[])
{
    queue<int> q;
    ...
    while(!q.empty())
    {
        ...
        {
            if(visited[graph[f][i]] == 0 && CDS[graph[f][i]] == 1)
            {
                q.push(graph[f][i]);
...
void set_routing_table(int n,vector<int> graph[],node nodes[])
{
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<n;j++)
        {
            if(nodes[i].sendTo(j) == -1)
            {
                int tmp = BFS(n,graph,j,i,CDS,nodes);
